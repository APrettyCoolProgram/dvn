...
internal static class DuDirectory
{
    ...
    // [250801]
    /// <summary>Verifies that a parent directory exists for the specified file path.</summary>
    ...
    /// <remarks>
    /// This method copies all files from <paramref name="source"/> to <paramref name="target"/>.
    /// If <paramref name="recursive"/> is <see langword="true"/>, subdirectories and their contents are copied as well.
    /// Existing files in the target directory are overwritten.
    /// </remarks>
    ...
    /// <exception cref="DirectoryNotFoundException">Thrown when <paramref name="source"/> does not exist.</exception>
    ...
    /// <remarks>
    /// This method ensures that the <paramref name="target"/> directory exists before copying.
    /// Files and directories specified in <paramref name="excludeFiles"/> and <paramref name="excludeDirs"/> are skipped during the copy operation.
    /// </remarks>
    ...
    /// <param name="recursive">A value indicating whether subdirectories should be copied recursively. <see langword="true"/> to copy subdirectories; otherwise, <see langword="false"/>.</param>
    /// <exception cref="DirectoryNotFoundException">Thrown when <paramref name="source"/> does not exist.</exception>
